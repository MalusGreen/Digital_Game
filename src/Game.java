...
//			}
	...
	public void clicked(Graphics g) {
		...
		g.fillOval(mx - click_count / 2 - map.getX(), my - click_count / 2 - map.getY(), click_count,
				click_count);
		click_count--;
	...
	public void dragBox(Graphics g) {
		...
		g.fillRect(rect.x-map.getX(), rect.y-map.getY(), rect.width, rect.height);
	}
	...
	public void setBackground(int size){
		...
		for(int i=0;i<pixels.length;i++){
			pixels[i]=new Pixel(new Color(255,255,255),map.getSect().rect.width,map.getSect().rect.height);
			pixels_2[i]=new Pixel(new Color(200,200,120),map.getSect().rect.width,map.getSect().rect.height);
		}
	}
